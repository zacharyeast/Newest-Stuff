...
PIN2_TRIGGER =  23
PIN2_ECHO = 29
GPIO.setup(PIN2_TRIGGER, GPIO.OUT)
GPIO.setup(PIN2_ECHO, GPIO.IN)
GPIO.output(PIN2_TRIGGER, GPIO.LOW)
GPIO.output(3, 1)
GPIO.output(7,False)
...
servo1 = GPIO.PWM(18,1000)
servo1.start(50)
def key_p(): #press p to rock the servo, then release 
    servo1.ChangeDutyCycle(75)
...
#         screen.blit(text_surface,(300,500))
        def rover_ultra_sense():
            ...
            GPIO.output(PIN2_TRIGGER, GPIO.HIGH)
            time.sleep(0.00001)
            GPIO.output(PIN2_TRIGGER, GPIO.LOW)
            ...
            while GPIO.input(PIN2_ECHO)==1:
                pulse_end_time = time.time()
            sleep(0.001)
            pulse_duration2 = pulse_end_time - pulse_start_time
            distance2 = round(pulse_duration2 * 17150, 2)
            print("Distance2:",distance2,"cm")
            distance4 = str(distance2)
            texttbd=textfont.render(distance4,1,(250,250,250))
            screen.blit(texttbd,(1,550))    
            pygame.display.flip()            
#            clock.tick(FPS)
            if distance <= safe_distance:
                ...
                sleep(0.1)
                main()
                distance5 = "Object Detected!"
                
                texttbd2=textfont.render(distance5,1,(250,250,250))

                screen.blit(texttbd2,(1,550))    
                pygame.display.flip()                 
                clock.tick(FPS)
            if distance2 <= safe_distance:
                print("OBJECT DETECTED AT THE Starboard")
                stop()
                sleep(0.1)
                main()
                distance5 = "Object Detected!"
